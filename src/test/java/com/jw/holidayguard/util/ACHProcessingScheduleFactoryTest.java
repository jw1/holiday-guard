...
class ACHProcessingScheduleFactoryTest {
    ...
    void shouldCreateACHScheduleDefinition() {
        ...
        assertEquals(ScheduleRules.RuleType.WEEKDAYS_ONLY, definition.getRule().getRuleType());
    ...
    void shouldCreateUpdateRequestForRestAPI() {
        ...
        UpdateScheduleRulesRequest updateRequest = definition.toUpdateRequest();
        
        assertNotNull(updateRequest);
        assertNotNull(updateRequest.getEffectiveFrom());
        assertEquals(1, updateRequest.getRules().size());
        assertEquals(11, updateRequest.getOverrides().size());
        
        // Verify rule details
        CreateScheduleRuleRequest rule = updateRequest.getRules().get(0);
        assertEquals(ScheduleRules.RuleType.WEEKDAYS_ONLY, rule.getRuleType());
