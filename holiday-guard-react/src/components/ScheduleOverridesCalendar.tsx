...
const ScheduleOverridesCalendar: React.FC<ScheduleOverridesCalendarProps> = ({ baseCalendar, initialOverrides, onOverridesChange }) => {
  ...
  const getDayClasses = (state: string) => {
    ...
    switch (state) {
      case 'run': return `${baseClasses} bg-green-100 text-green-800`;
      case 'no-run': return `${baseClasses} bg-red-100 text-red-800`;
      case 'FORCE_RUN': return `${baseClasses} bg-green-200 text-green-900 ring-2 ring-green-500`;
      case 'SKIP': return `${baseClasses} bg-red-200 text-red-900 ring-2 ring-red-500`;
